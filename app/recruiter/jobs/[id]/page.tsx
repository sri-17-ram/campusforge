...
import { ArrowLeft, Clock, MapPin, Download, CheckCircle, ShieldCheck, Star, ExternalLink, Calendar, FileText, X, User, Activity, Briefcase, Award } from 'lucide-react'
...
const STATUSES = ['APPLIED', 'SCREENING', 'TECHNICAL_ROUND', 'HR_ROUND', 'OFFER', 'ACCEPTED', 'REJECTED', 'HIRED']
...
export default function ATSBoardPage() {
  ...
  const [interviewData, setInterviewData] = useState({
    date: '', time: '', mode: 'ONLINE', link: '', location: '', interviewer: '', round: '1'
  })
  ...
  const scheduleInterview = async () => {
    if (!interviewData.date || !interviewData.time) return alert("Date and time required")
    const token = localStorage.getItem('token')
    
    // combine date and time
    const dateTime = new Date(`${interviewData.date}T${interviewData.time}`).toISOString()
  ...
  const handleDocumentUpload = async (e: React.ChangeEvent<HTMLInputElement>, docType: string) => {
    ...
    try {
      ...
      if (!presignedData.success) throw new Error(presignedData.message)
      ...
      // 3. Save URL to Application Document Mapping
      const assignRes = await fetch('/api/recruiter/applications/documents', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          applicationId: selectedApp.id,
          documentType: docType,
          fileUrl: presignedData.finalUrl
        })
      })
      
      if (assignRes.ok) {
        fetchJob()
        // Refresh selected app manually to immediately show link
        setSelectedApp({ ...selectedApp, [docType]: presignedData.finalUrl })
      }
    } catch (err) {
      console.error(err)
  ...
  if (!job) {
    return <AppLayout><div className="text-center p-12 text-muted-foreground">Job not found.</div></AppLayout>
  }
      ...
      <div className="space-y-6 h-[calc(100vh-100px)] flex flex-col">
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
          <div className="flex h-full gap-4 min-w-max">
            {STATUSES.map(status => {
              const apps = job.applications?.filter((a: any) => a.status === status) || []
              return (
                <div 
                  key={status} 
                  className="w-80 h-full flex flex-col bg-muted/20 border border-muted/40 rounded-xl overflow-hidden"
                  onDragOver={handleDragOver}
                  onDrop={(e) => handleDrop(e, status)}
                >
                  <div className="p-3 border-b border-muted/40 bg-card flex justify-between items-center shadow-sm z-10">
                    <h3 className="font-semibold text-sm text-foreground">{status.replace('_', ' ')}</h3>
                    <span className="bg-primary/10 text-primary text-xs px-2 py-0.5 rounded-full font-bold">{apps.length}</span>
                  </div>
                  <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                    {apps.map((app: any) => (
                      <div
                        ...
                        className="p-4 bg-card rounded-lg border border-primary/20 shadow-sm cursor-grab hover:shadow-md hover:border-primary/50 transition-all group relative"
                      >
                        <h4 className="font-bold text-foreground mb-1 group-hover:text-primary transition-colors">{app.student?.user?.name || 'Unknown'}</h4>
                        <p className="text-xs text-muted-foreground mb-3">{app.student?.branch || 'General'} • {app.student?.year ? `Year ${app.student.year}` : ''}</p>
                        
                        <div className="flex flex-wrap gap-2 text-[10px]">
                          {app.student?.skills?.slice(0, 3).map((s: any) => (
                            <span key={s.id} className="bg-muted px-2 py-1 rounded-sm">{s.name}</span>
                          ))}
                          {(app.student?.skills?.length || 0) > 3 && <span className="bg-muted px-2 py-1 rounded-sm">+{app.student.skills.length - 3}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="p-6 border-b border-muted bg-card flex justify-between items-center shrink-0">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-muted flex items-center justify-center overflow-hidden border-2 border-primary/20 shrink-0">
                   {selectedApp?.student?.user?.avatar ? <Image width={500} height={500} src={selectedApp.student.user.avatar} alt="avatar" className="w-full h-full object-cover"/> : <User className="w-8 h-8 text-muted-foreground" />}
                </div>
                <div>
